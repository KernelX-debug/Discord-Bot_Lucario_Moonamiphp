...
class MoonaniClient:
    ...
    def search_zero_iv_pokemon(
        ...
        while total_records is None or start < total_records:
            ...
            page = self._fetch_page(start=start, length=page_size, iv_filter=0, pvp=0)
            rows = page.get("data", [])
            total_records = _safe_int(page.get("recordsFiltered"))
            ...
            for row in rows:
                ...
                if normalized_query and normalized_query not in _normalize_name(spawn.name):
                    continue
                if not spawn.is_zero_iv and spawn.iv_percent != 0:
                    continue
    ...
    def list_current_zero_iv_spawns(
        self,
        limit: int = 250,
        page_size: int = 100,
        max_records: Optional[int] = 2000,
    ) -> List[PokemonSpawn]:
        return self.search_zero_iv_pokemon(
            query="",
            limit=limit,
            page_size=page_size,
            max_records=max_records,
