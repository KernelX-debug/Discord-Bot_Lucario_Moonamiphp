...
def _build_list_embed(results: List[PokemonSpawn], query: str) -> discord.Embed:
...
def register_commands(bot: LucarioDiscordBot) -> None:
    ...
    async def pokemon(
        ...
        try:
            results = await _search_spawns(bot, nombre, cantidad)
        ...
        if len(results) == 1:
            await interaction.followup.send(embed=_build_detail_embed(results[0]))
            return

        await interaction.followup.send(
            embed=_build_list_embed(results, query=nombre or ""),
        )
